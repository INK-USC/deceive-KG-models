...
def main2(mode = 'train', model = None):
    ...
    if args.test_statements is not None:
        find_relational_paths(args.cpnet_vocab_path, args.cpnet_graph_path, args.test_concepts, args.test_rel_paths, args.nprocs, True)
...
def eval(args, model = None):
    ...
    dev_acc,acc = evaluate_accuracy(dataset.dev(), model)
    #dev_acc = 0
    #test_acc = evaluate_accuracy(dataset.test(), model) if dataset.test_size() else 0.0
    test_acc = 0
    print("***** evaluation done *****")
    print(np.mean(dev_acc),np.sum(acc))
