...
class LMGraphRelationNetDataLoader(object):
    ...
    def __init__(self, train_statement_path, train_adj_path,
                 ...
                 train_embs_path=None, dev_embs_path=None, test_embs_path=None, ratio=1,
        ...
        *self.dev_decoder_data, self.dev_adj_data, n_rel = load_adj_data(dev_adj_path, max_node_num, num_choice, emb_pk_path=dev_embs_path if use_contextualized else None)
        ...
        data=[]
        if self.ratio < 1:
            with open('train_'+str(int(self.ratio*100))+'.jsonl') as f:
                for line in f:
                    data.append(json.loads(line)['id'])
            self.inhouse_train_indexes = torch.tensor([i for i, qid in enumerate(self.train_qids) if qid in data])
