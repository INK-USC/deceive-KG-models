...
from utils.relpath_utils import *
import itertools
from new_val_score import *
from test_fact_validation import *
from newer_val_score import *
...
#             logits, _ = model(*input_data, layer_id=-1, edge_index=edge_index, c_ids=c_ids, u=None, batch=batch,
#                               rel_ids=rel_ids, num_tuples=num_tuples)
...
def main2(mode='train', model=None):
    parser = get_parser()
    ...
    find_relational_paths(args.cpnet_vocab_path, args.cpnet_graph_path, args.train_concepts, args.train_rel_paths,
                          args.nprocs, args.use_cache)
    find_relational_paths(args.cpnet_vocab_path, args.cpnet_graph_path, args.dev_concepts, args.dev_rel_paths,
                          args.nprocs, args.use_cache)
    if args.test_statements is not None:
        find_relational_paths(args.cpnet_vocab_path, args.cpnet_graph_path, args.test_concepts, args.test_rel_paths,
                              args.nprocs, args.use_cache)
...
def train(args):
    ...
    LMdataset = LMGraphNetDataLoader(args.train_statements, args.train_rel_paths,
                                     ...
                                     batch_size=args.mini_batch_size, eval_batch_size=args.eval_batch_size,
                                     device=device,
    ...
    GraphDataset = GraphDataLoader(args.train_adj, args.dev_adj, args.test_adj, args.mini_batch_size,
                                   args.eval_batch_size, args.inhouse)
    ...
    for epoch_id in range(args.n_epochs):
        ...
        for graph, data in zip(train_loader, LMdataset.train(train_index)):
            ...
            # for a in range(0, bs, args.mini_batch_size):
            #    b = min(a + args.mini_batch_size, bs)
            logits, _ = model(*input_data, layer_id=args.encoder_layer, edge_index=edge_index, c_ids=c_ids, u=None,
                              batch=batch, rel_ids=rel_ids, num_tuples=num_tuples)
            # print('k')
            if args.loss == 'margin_rank':
            ...
            if (i + args.mini_batch_size) % args.batch_size == 0 or (i + args.mini_batch_size) == train_size:
                ...
                if (global_step + 1) % args.log_interval == 0:
                    ...
                    print('| step {:5} |  lr: {:9.7f} | loss {:7.4f} | ms/batch {:7.2f} | batch{:5}  |'.format(
                        global_step, scheduler.get_lr()[0], total_loss, ms_per_batch, i))
        ...
        test_acc = evaluate_accuracy(test_loader, LMdataset.test(test_index), model,
                                     device) if args.test_statements else 0.0
        print('-' * 71)
...
def eval(args, model=None):
    ...
    # dev_acc = evaluate_accuracy(dev_loader, LMdataset.dev(), model, device)
    # dev_acc = 0
    # test_acc = evaluate_accuracy(test_loader, LMdataset.test(test_index), model, device)
    # test_acc = 0
    dev_acc,acc = evaluate_accuracy(dev_loader, LMdataset.dev(), model, device)
    print("***** evaluation done *****")
    # print()
    # print(f'| dev_accuracy: {dev_acc} | test_acc: {test_acc} |')
    print(np.mean(dev_acc),np.sum(acc), len(acc))
    return dev_acc,acc
