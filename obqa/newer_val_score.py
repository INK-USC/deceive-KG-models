...
def get_dist2():
    ori_stats = np.load('degree_stats.npy')
    ...
    for rel_id in id2relation:
        ...
        for node, val in list(cc):
            #print(node, val)
            coeff[relation2id[rel_id], node] = val
        #coeff.append(torch.FloatTensor(list(cc.values())))
        #print(len(list(cc.values())))
    #new_stats = []
    # for i in range(17):
    ...
    #print([(ori_stats[i].shape) for i in range(len(ori_stats))])
    cv = np.sqrt(np.sum((coeff - ori_stats)**2, axis=1))
    print(cv.shape)
    print(1/(np.mean(cv) + 0.00001))
    #np.save('degree_stats.npy', coeff)
