...
from utils.graph_grn import generate_graph, generate_adj_data_from_grounded_concepts, coo_to_normalized
...
def main5():
    ...
    parser1.add_argument('-np', '--num_pert', type=int, default=0, help='number of processes to use')
    parser1.add_argument('--type', type=str, default='rel', help='threshold for pruning paths')
    ...
    routines = {
        ...
        'csqa': [
            ...
            {'func': convert_to_entailment, 'args': (input_paths['csqa']['dev'], output_paths['csqa']['statement']['dev'], False, True)},
...
#             {'func': generate_path_and_graph_from_adj, 'args': (output_paths['obqa']['graph']['adj-test'], output_paths['cpnet']['pruned-graph'], output_paths['obqa']['paths']['adj-test'], output_paths['obqa']['graph']['nxg-from-adj-test'], args.nprocs)},
    ...
    perturb_cpnet('./data/cpnet/conceptnet_2.en.pruned.graph', args.num_pert, './data/cpnet/conceptnet.en.pruned.graph', type=args.type)
    for rt in args.run:
