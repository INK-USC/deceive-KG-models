...
def open_graph(graph_path):
    ...
    for graph in tqdm(graph_list):
        ...
        edge_index = torch.tensor(np.concatenate((row.reshape(1, row.shape[0]), col.reshape(1, col.shape[0])), axis=0),
                                  dtype=torch.long)
...
class GraphDataLoader(object):
    def __init__(self, train_adj_path, dev_adj_path, test_adj_path, batch_size, eval_batch_size, is_inhouse):
        self.batch_size = 4 * batch_size
        self.eval_batch_size = 4 * eval_batch_size
    ...
    def train_graph_data(self, train_indexes=None):
        train_data = []
        if self.is_inhouse:
    ...
    def test_graph_data(self, test_indexes=None):
        test_data = []
        if self.is_inhouse:
...
class Decoder(nn.Module):
    ...
    def __init__(self, concept_num, concept_dim, relation_num, relation_dim, sent_dim, concept_in_dim,
        ...
        self.GraphNet = GraphNet(concept_num, concept_dim, relation_num, relation_dim, sent_dim, concept_in_dim,
                                 hidden_size, num_hidden_layers, num_attention_heads, fc_size, num_fc_layers, dropout,
                                 pretrained_concept_emb=pretrained_concept_emb,
                                 pretrained_relation_emb=pretrained_relation_emb, freeze_ent_emb=freeze_ent_emb,
                                 init_range=init_range, ablation=ablation, use_contextualized=use_contextualized,
                                 emb_scale=emb_scale)
...
class LMGraphNet(nn.Module):
    ...
    def __init__(self, model_name,
        ...
        self.decoder = Decoder(concept_num, concept_dim, relation_num, relation_dim, self.encoder.sent_dim,
                               concept_in_dim,
    ...
    def forward(self, *inputs, layer_id, edge_index, c_ids, u, batch, rel_ids, num_tuples):
        ...
        *lm_inputs, = inputs
        ...
        logits, attn = self.decoder(sent_vecs=sent_vecs, edge_index=edge_index, c_ids=c_ids, u=u, batch=batch,
                                    num_tuples=num_tuples, rel_ids=rel_ids)  # cxy-style param passing
...
class LMGraphNetDataLoader(object):
    ...
    def __init__(self, train_statement_path, train_rpath_jsonl,
        ...
        self.train_qids, self.train_labels, *self.train_data = load_input_tensors(train_statement_path, model_type,
                                                                                  model_name, max_seq_length,
                                                                                  format=format)
        self.dev_qids, self.dev_labels, *self.dev_data = load_input_tensors(dev_statement_path, model_type, model_name,
                                                                            max_seq_length, format=format)
        ...
        if test_statement_path is not None:
            self.test_qids, self.test_labels, *self.test_data = load_input_tensors(test_statement_path, model_type,
                                                                                   model_name, max_seq_length,
                                                                                   format=format)
    ...
    def train_eval(self):
        return BatchGenerator(self.device, self.eval_batch_size, torch.arange(len(self.train_qids)), self.train_qids,
                              self.train_labels, tensors=self.train_data)

    def dev(self):
        return BatchGenerator(self.device, self.eval_batch_size, torch.arange(len(self.dev_qids)), self.dev_qids,
                              self.dev_labels, tensors=self.dev_data)

    def test(self, index):
        if self.is_inhouse:
            return BatchGenerator(self.device, self.eval_batch_size, index, self.train_qids, self.train_labels,
                                  tensors=self.train_data)
        else:
            return BatchGenerator(self.device, self.eval_batch_size, torch.arange(len(self.test_qids)), self.test_qids,
                                  self.test_labels, tensors=self.test_data)
