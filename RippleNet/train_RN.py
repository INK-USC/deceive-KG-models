...
import numpy as np
from model_RN import RippleNet

def eval_acc(args, data_info, show_loss, show_topk=False):
    ...
    with tf.Session() as sess:
        oSaver.restore(sess, 'model_movie.ckpt')
        #sess = KGCN['oSess']
        eval_auc, eval_f1 = evaluation(sess, args, model, eval_data, ripple_set, args.batch_size)
        print('evaluation 1 done')
        test_auc, test_f1 = evaluation(sess, args, model, test_data, ripple_set, args.batch_size)
    print('  eval auc: %.4f  f1: %.4f    test auc: %.4f  f1: %.4f'
          % (eval_auc, eval_f1, test_auc, test_f1), flush=True)
    return eval_auc
...
def train(args, data_info, show_loss):
    ...
    with tf.Session() as sess:
        ...
        for step in range(args.n_epoch):
            ...
            if eval_auc > best_dev_auc:
                best_dev_auc = eval_auc
                oSaver = tf.train.Saver()
                oSess = sess
                oSaver.save(oSess, 'model_movie.ckpt')
                print("model_saved")
