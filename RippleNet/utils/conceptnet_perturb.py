...
def construct_graph(cpnet_csv_path, cpnet_vocab_path, output_path, num_changes=0, prune=True):
    ...
    with open(cpnet_csv_path, "r", encoding="utf8") as fin:
        ...
        for line in tqdm(fin, total=nrow):
            ...
            if (subj, obj, rel) not in attrs:
                ...
                graph.add_edge(obj, subj, rel=(rel + len(relation2id))%(2*len(relation2id)), weight=weight)
                attrs.add((obj, subj, (rel + len(relation2id))%(2*len(relation2id))))
