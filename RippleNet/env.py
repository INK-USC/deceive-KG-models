...
def get_prob_cost(prob_list):
    ...
    for li in prob_list:
        ...
        for i in range(4):
            if i!=co:
                prob_cost+=math.log(0.25/li[i])/4
...
class NodeAttakEnv(object):
    ...
    def reset(self):
        ...
        self.prev_dev_auc = self.init_avg_prob
        # with open('./data/cpnet/conceptnet_2.en.pruned.graph', 'rb') as handle:
        #     self.graph = pickle.load(handle)
        kg_np = np.load("../data/" + self.args.dataset + "/kg_initial.npy")
        self.graph = npy2nx(kg_np, "../data/cpnet/conceptnet_2.en.pruned.graph")
    ...
    def predict_score(self,actions0,actions1):
        ...
        for i in range(32):
            ...
            x[:, 1] = i
            score.append(pred(x[0,0], x[0,1],x[0,2]))
    ...
    def step(self, action, state, hc, model=None):
        ...
        if self.args.model_id not in [4,5,6,7]:
            ...
            act_score = pred(x[0,0], x[0,1],x[0,2])
            ...
            changed_act_score = pred(x[0,0], x[0,1], x[0,2])
        ...
        state[2]=state[2]+1
        state[3]=hc
        if ((self.n_steps+1)%self.update_step == 0 and self.n_steps > 0) or self.n_steps == 499000:
            state[2].zero_()
            state[3]=self.hc0
            state[1].append(action)

            output_path = "../data/cpnet/conceptnet.en.pruned.graph"
            output_npy_path = "../data/" + self.args.dataset + "/kg_final_" + str(self.n_steps+1) +".npy"
            #output_npy_path = "../data/" + self.args.dataset + "/kg_final.npy"
            #self.graph = state[0]
            #state[0] = self.graph
            ...
            kg_np = nx2npy(self.graph, output_npy_path)
            if self.eval or self.args.debug_mode:
                dev_auc=self.prev_dev_auc
                acc=0
                prob_list=[]
                acc_list=[]
            else:
                ...
                # main1()
                # prob_list,acc_list = main2(model=model)
                dev_auc = main2(model = model)
            ...
            reward = (dev_auc - self.prev_dev_auc) #+ 1000*(self.update_step_score - self.score)[0]
            # print('reward0=',reward, avg_prob, self.prev_avg_prob)
            state[1] = []
            self.update_step_score = self.score
            self.prev_dev_auc = dev_auc
            # self.prev_acc = acc
        else:
        ...
        return state, reward, self.is_Terminal(), self.score, dev_auc, 0, [], self.graph2
